...
with uproot.open(FILE) as f:
    ...
    for board in range(4):
        ...
        # --------------------------------------------------------
        # Histogram & Gaussian Fit (non-zero Δt50)
        # --------------------------------------------------------

        delta_valid = delta_t  # already masked

        if len(delta_valid) == 0:
            print("No valid Δt values — skipping histogram.")
            continue
        ...
        plt.hist(delta_valid, bins=2000, density=True,
                 histtype="step", linewidth=1.4, alpha=0.8,
                 label="Δt50 (valid)")

        x = np.linspace(delta_valid.min(), delta_valid.max(), 400)
        plt.plot(x, norm.pdf(x, mu, sigma),
                 "k--", linewidth=1.5,
                 label=f"Gaussian Fit\nμ={mu:.3f} ns, σ={sigma:.3f} ns")
        ...
        print(f"Board {board}: DONE — saved histogram.")
