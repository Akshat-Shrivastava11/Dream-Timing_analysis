...
def process_and_plot_channel(files, drs_code, fers_code, family_name, pdf, tmin, tmax, emin, emax, nbins, particle_type, pid_tag):
    """Processes a single channel across all files and adds its 3 combined plots to the master PDF."""
    db, dg, dc = _parse_code(drs_code)
    fb, fc = int(fers_code[:-2]), int(fers_code[-2:])
    ...
    for fpath in files:
        try:
            ...
            pass # Suppress individual file errors to keep console clean during the big loop
    ...
    if len(all_t_valid) > 0:
        ...
        corr_val = np.corrcoef(all_t_valid, all_e_valid)[0, 1] if len(all_t_valid) > 1 else 0.0
        ax.legend(handles=[mpatches.Patch(color='none', label=f"Correlation (r): {corr_val:.3f}")], loc='upper right', fontsize=12)
        
        ax.set_title(f"[{family_name}] Energy vs ToA: DRS {drs_code} $\\rightarrow$ FERS {fers_code}\nAll Runs Combined (PID: {pid_tag})", fontsize=14)
        ax.set_xlabel(r"$|t_{\mathrm{final}}|$ [ns]", fontsize=12)
        ax.set_ylabel("Energy [A.U.]", fontsize=12)
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
        ...
        # --- 2. PROFILE 1 (Energy Binned -> Median Time) ---
        fig2, ax2 = plt.subplots(figsize=(10, 8))
        energy_bins = np.linspace(emin, emax, n_profile_bins + 1)
        e_centers, t_medians, t_errs = [], [], []
        ...
        if len(e_centers) > 0:
            ...
            #     m, b = popt[0], popt[1]
            #     sign = "+" if b >= 0 else "-"
            #     ax2.plot(t_smooth, e_smooth, color='red', lw=2.5, label=f"Linear Fit: $t = {m:.2e}E {sign} {abs(b):.2f}$", zorder=4)

        # --- NEW EXPONENTIAL FIT (Time vs Energy) ---
            if len(e_centers) > 2: # Need at least 3 points for a 2-parameter fit
                try:
                    # Initial guess: A ~ max time, lambda ~ mean energy
                    p0 = [np.max(t_medians), np.mean(e_centers) if np.mean(e_centers) != 0 else 1.0]
                    popt, _ = curve_fit(exp_func, e_centers, t_medians, p0=p0, maxfev=10000)
        ...
        # --- 3. PROFILE 2 (Time Binned -> Median Energy) ---
        fig3, ax3 = plt.subplots(figsize=(10, 8))
        time_bins = np.linspace(tmin, tmax, n_profile_bins + 1)
        t_centers, e_medians, e_errs = [], [], []
        ...
        if len(t_centers) > 0:
            ...
            #     m, b = popt3[0], popt3[1]
            #     sign = "+" if b >= 0 else "-"
            #     ax3.plot(t_smooth3, e_smooth3, color='orange', lw=2.5, label=f"Linear Fit: $E = {m:.2e}t {sign} {abs(b):.2f}$", zorder=4)

            # --- NEW EXPONENTIAL FIT (Energy vs Time) ---
            if len(t_centers) > 2:
                try:
                    # Initial guess: A ~ max energy, lambda ~ mean time
                    p0 = [np.max(e_medians), np.mean(t_centers) if np.mean(t_centers) != 0 else 1.0]
                    popt3, _ = curve_fit(exp_func, t_centers, e_medians, p0=p0, maxfev=10000)
...
def make_master_correlation_pdf(files, outdir, tmin, tmax, emin, emax, nbins, particle_type=None):
    ...
    with PdfPages(out_pdf) as pdf:
        for family_name, grid in families:
            ...
            # --- OVERRIDE TIMING FOR SCINTILLATOR ---
            ...
            for row in grid:
                for drs_code in row:
                    ...
                    # Process and plot this specific channel using the active time limits
                    process_and_plot_channel(
                        files, drs_code, fers_code, family_name, pdf, 
                        active_tmin, active_tmax, a, emax, nbins, particle_type, pid_tag
                    )
